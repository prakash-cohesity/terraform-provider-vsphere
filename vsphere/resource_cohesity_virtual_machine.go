...
func resourceCohesityVirtualMachine() *schema.Resource {
	...
	return &schema.Resource{
		Create:        resourceVSphereCohesityVirtualMachineCreate,
		Read:          resourceVSphereCohesityVirtualMachineRead,
		Update:        resourceVSphereCohesityVirtualMachineUpdate,
		Delete:        resourceVSphereCohesityVirtualMachineDelete,
		CustomizeDiff: resourceVSphereCohesityVirtualMachineCustomizeDiff,
		Importer: &schema.ResourceImporter{
			State: resourceVSphereCohesityVirtualMachineImport,
		},
		SchemaVersion: 3,
...
func resourceVSphereCohesityVirtualMachineCreate(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] %s: Beginning create", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	///////////////////////////////////////////
	// Set the datastore for the VM.
	_, err := datastore.FromID(client, d.Get("nas_datastore_id").(string))
	if err != nil {
		_, err = datastore.FromID(client, d.Get("datastore_id").(string))
		if err != nil {
			return fmt.Errorf("error locating datastore for VM: %s", err)
		}
	}
	// Update the value to NAS datastore
	err = d.Set("datastore_id", d.Get("nas_datastore_id").(string))
	if err != nil {
		return fmt.Errorf("==PV== error locating datastore for VM: %s", err)
	}
	disks := d.Get("disk").([]interface{})
	for _, disk := range disks {
		diskTemp := disk.(map[string]interface{})
		diskTemp["datastore_id"] = d.Get("nas_datastore_id")
		log.Printf("[DEBUG] %+v: DISKS", disk)
		log.Printf("[DEBUG] %#v: DISKS", disk)
	}
	///////////////////////////////////////////
	...
	vm, err = resourceVSphereCohesityVirtualMachineCreateBare(d, meta)
	...
	if hid, ok := d.GetOk("host_system_id"); hid.(string) != vprops.Runtime.Host.Reference().Value && ok {
		err = resourceVSphereCohesityVirtualMachineRead(d, meta)
		if err != nil {
			return err
		}
		...
		if err = resourceVSphereCohesityVirtualMachineUpdateLocation(d, meta); err != nil {
			return err
		}
	}
	...
	// All done!
	log.Printf("[DEBUG] %s: Create complete", resourceVSphereCohesityVirtualMachineIDString(d))
	return resourceVSphereCohesityVirtualMachineRead(d, meta)
}

func resourceVSphereCohesityVirtualMachineRead(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] %s: Reading state of virtual machine", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	id := d.Id()
	vm, err := virtualmachine.FromUUID(client, id)
	if err != nil {
		if _, ok := err.(*virtualmachine.UUIDNotFoundError); ok {
			log.Printf("[DEBUG] %s: Virtual machine not found, marking resource as gone: %s", resourceVSphereCohesityVirtualMachineIDString(d), err)
			d.SetId("")
			return nil
		}
	...
	log.Printf("[DEBUG] %s: Read complete", resourceVSphereCohesityVirtualMachineIDString(d))
	return nil
}

func resourceVSphereCohesityVirtualMachineUpdate(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] %s: Performing update", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	tagsClient, err := tagsClientIfDefined(d, meta)
	if err != nil {
		return err
	...
	if changed || len(spec.DeviceChange) > 0 {
		...
		if _, ok := d.GetOk("datastore_cluster_id"); ok {
			err = resourceVSphereCohesityVirtualMachineUpdateReconfigureWithSDRS(d, meta, vm, spec)
		} else {
	...
	log.Printf("[DEBUG] %s: Update complete", resourceVSphereCohesityVirtualMachineIDString(d))
	return resourceVSphereCohesityVirtualMachineRead(d, meta)
}

// resourceVSphereCohesityVirtualMachineUpdateReconfigureWithSDRS runs the reconfigure
// part of resourceVSphereCohesityVirtualMachineUpdate through storage DRS. It's
// designed to be run when a storage cluster is specified, versus simply
// specifying datastores.
func resourceVSphereCohesityVirtualMachineUpdateReconfigureWithSDRS(
	...
	log.Printf("[DEBUG] %s: Reconfiguring virtual machine through Storage DRS API", resourceVSphereCohesityVirtualMachineIDString(d))
...
func resourceVSphereCohesityVirtualMachineDelete(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] %s: Performing delete", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	id := d.Id()
	vm, err := virtualmachine.FromUUID(client, id)
	if err != nil {
	...
	d.SetId("")
	log.Printf("[DEBUG] %s: Delete complete", resourceVSphereCohesityVirtualMachineIDString(d))
	return nil
}

func resourceVSphereCohesityVirtualMachineCustomizeDiff(d *schema.ResourceDiff, meta interface{}) error {
	log.Printf("[DEBUG] %s: Performing diff customization and validation", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	...
	if err := resourceVSphereCohesityVirtualMachineCustomizeDiffResourcePoolOperation(d); err != nil {
		return err
	}
	...
	log.Printf("[DEBUG] %s: Diff customization and validation complete", resourceVSphereCohesityVirtualMachineIDString(d))
	return nil
}

func resourceVSphereCohesityVirtualMachineCustomizeDiffResourcePoolOperation(d *schema.ResourceDiff) error {
	if d.HasChange("resource_pool_id") && !d.HasChange("host_system_id") {
		log.Printf(
			...
			resourceVSphereCohesityVirtualMachineIDString(d),
		)
...
func resourceVSphereCohesityVirtualMachineImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	client := meta.(*VSphereClient).vimClient
	...
	rs := resourceCohesityVirtualMachine().Schema
	...
	log.Printf("[DEBUG] %s: Import complete, resource is ready for read", resourceVSphereCohesityVirtualMachineIDString(d))
	return []*schema.ResourceData{d}, nil
}

// resourceVSphereCohesityVirtualMachineCreateBare contains the "bare metal" VM
// deploy path. The VM is returned.
func resourceVSphereCohesityVirtualMachineCreateBare(d *schema.ResourceData, meta interface{}) (*object.VirtualMachine, error) {
	log.Printf("[DEBUG] %s: VM being created from scratch", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	...
	var vm *object.VirtualMachine
	if _, ok := d.GetOk("datastore_cluster_id"); ok {
		vm, err = resourceVSphereCohesityVirtualMachineCreateBareWithSDRS(d, meta, fo, spec, pool, hs)
	} else {
		vm, err = resourceVSphereCohesityVirtualMachineCreateBareStandard(d, meta, fo, spec, pool, hs)
	}
	if err != nil {
		return nil, err
	}
...
// resourceVSphereCohesityVirtualMachineCreateBareWithSDRS runs the creation part of
// resourceVSphereCohesityVirtualMachineCreateBare through storage DRS. It's designed
// to be run when a storage cluster is specified, versus simply specifying
// datastores.
func resourceVSphereCohesityVirtualMachineCreateBareWithSDRS(
	d *schema.ResourceData,
	meta interface{},
	fo *object.Folder,
	spec types.VirtualMachineConfigSpec,
	...
	log.Printf("[DEBUG] %s: Creating virtual machine through Storage DRS API", resourceVSphereCohesityVirtualMachineIDString(d))
	pod, err := storagepod.FromID(client, d.Get("datastore_cluster_id").(string))
	if err != nil {
...
// resourceVSphereCohesityVirtualMachineCreateBareStandard performs the steps necessary
// during resourceVSphereCohesityVirtualMachineCreateBare to create a virtual machine
// when a datastore cluster is not supplied.
func resourceVSphereCohesityVirtualMachineCreateBareStandard(
	d *schema.ResourceData,
	meta interface{},
	fo *object.Folder,
	spec types.VirtualMachineConfigSpec,
...
// resourceVSphereCohesityVirtualMachineCreateClone contains the clone VM deploy
// path. The VM is returned.
func resourceVSphereCohesityVirtualMachineCreateClone(d *schema.ResourceData, meta interface{}) (*object.VirtualMachine, error) {
	log.Printf("[DEBUG] %s: VM being created from clone", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	...
	var vm *object.VirtualMachine
	if _, ok := d.GetOk("datastore_cluster_id"); ok {
		vm, err = resourceVSphereCohesityVirtualMachineCreateCloneWithSDRS(d, meta, srcVM, fo, name, cloneSpec, timeout)
	} else {
	...
	vprops, err := virtualmachine.Properties(vm)
	if err != nil {
		return nil, resourceVSphereCohesityVirtualMachineRollbackCreate(
			d,
			meta,
			vm,
	...
	cfgSpec.DeviceChange = virtualdevice.AppendDeviceChangeSpec(cfgSpec.DeviceChange, delta...)
	log.Printf("[DEBUG] %s: Final device list: %s", resourceVSphereCohesityVirtualMachineIDString(d), virtualdevice.DeviceListString(devices))
	log.Printf("[DEBUG] %s: Final device change cfgSpec: %s", resourceVSphereCohesityVirtualMachineIDString(d), virtualdevice.DeviceChangeString(cfgSpec.DeviceChange))

	// Perform updates
	if _, ok := d.GetOk("datastore_cluster_id"); ok {
		err = resourceVSphereCohesityVirtualMachineUpdateReconfigureWithSDRS(d, meta, vm, cfgSpec)
	} else {
	...
	if len(d.Get("clone.0.customize").([]interface{})) > 0 {
		...
		if err := virtualmachine.Customize(vm, custSpec); err != nil {
			...
			if derr := resourceVSphereCohesityVirtualMachineDelete(d, meta); derr != nil {
	...
	if cw != nil {
		log.Printf("[DEBUG] %s: Waiting for VM customization to complete", resourceVSphereCohesityVirtualMachineIDString(d))
		<-cw.Done()
...
// resourceVSphereCohesityVirtualMachineCreateCloneWithSDRS runs the clone part of
// resourceVSphereCohesityVirtualMachineCreateClone through storage DRS. It's designed
// to be run when a storage cluster is specified, versus simply specifying
// datastores.
func resourceVSphereCohesityVirtualMachineCreateCloneWithSDRS(
	d *schema.ResourceData,
	meta interface{},
	...
	log.Printf("[DEBUG] %s: Cloning virtual machine through Storage DRS API", resourceVSphereCohesityVirtualMachineIDString(d))
	pod, err := storagepod.FromID(client, d.Get("datastore_cluster_id").(string))
	if err != nil {
		return nil, fmt.Errorf("error getting datastore cluster: %s", err)
	}
...
// resourceVSphereCohesityVirtualMachineRollbackCreate attempts to "roll back" a
...
func resourceVSphereCohesityVirtualMachineRollbackCreate(
	d *schema.ResourceData,
	meta interface{},
	vm *object.VirtualMachine,
	origErr error,
	...
	if err := resourceVSphereCohesityVirtualMachineDelete(d, meta); err != nil {
...
// resourceVSphereCohesityVirtualMachineUpdateLocation manages vMotion. This includes
...
func resourceVSphereCohesityVirtualMachineUpdateLocation(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] %s: Checking for pending migration operations", resourceVSphereCohesityVirtualMachineIDString(d))
	client := meta.(*VSphereClient).vimClient
	...
	if !d.HasChange("resource_pool_id") && !d.HasChange("host_system_id") && !d.HasChange("datastore_id") && !diskRelocateOK {
		log.Printf("[DEBUG] %s: No migration operations found", resourceVSphereCohesityVirtualMachineIDString(d))
		return nil
	}
	log.Printf("[DEBUG] %s: Migration operations found, proceeding with migration", resourceVSphereCohesityVirtualMachineIDString(d))
	...
	if _, ok := d.GetOk("datastore_cluster_id"); ok {
		err = resourceVSphereCohesityVirtualMachineUpdateLocationRelocateWithSDRS(d, meta, vm, spec, timeout)
	} else {
...
// resourceVSphereCohesityVirtualMachineUpdateLocationRelocateWithSDRS runs the storage vMotion
// part of resourceVSphereCohesityVirtualMachineUpdateLocation through storage DRS.
// It's designed to be run when a storage cluster is specified, versus simply
// specifying datastores.
func resourceVSphereCohesityVirtualMachineUpdateLocationRelocateWithSDRS(
	d *schema.ResourceData,
	meta interface{},
	vm *object.VirtualMachine,
	spec types.VirtualMachineRelocateSpec,
	...
	log.Printf("[DEBUG] %s: Running virtual machine relocate Storage DRS API", resourceVSphereCohesityVirtualMachineIDString(d))
	pod, err := storagepod.FromID(client, d.Get("datastore_cluster_id").(string))
	if err != nil {
		return fmt.Errorf("error getting datastore cluster: %s", err)
	}
...
// resourceVSphereCohesityVirtualMachineIDString prints a friendly string for the
// vsphere_virtual_machine resource.
func resourceVSphereCohesityVirtualMachineIDString(d structure.ResourceIDStringer) string {
